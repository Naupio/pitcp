...
start_link(Ref, ListenSocket, ProMod, ProModOpt,
...
init([Ref, ListenSocket, ProMod, ProModOpt, OtherOpt,
      ListenerSup]) ->
    AcceptorNum = case lists:keyfind(acceptor_num, 1,
				     OtherOpt)
		      of
		    false -> 10;
		    {acceptor_num, Value} -> Value
		  end,
    ChildSpec = acceptor_generator([AcceptorNum, Ref,
				    ListenSocket, ProMod, ProModOpt, OtherOpt,
				    ListenerSup]),
    SupFlags = #{strategy => one_for_one, intensity => 1,
		 period => 5},
    {ok, {SupFlags, ChildSpec}}.

acceptor_generator([AcceptorNum, Ref, ListenSocket,
		    ProMod, ProModOpt, OtherOpt, ListenerSup])
    when AcceptorNum > 0 andalso is_integer(AcceptorNum) ->
    lists:map(fun (NumIndex) ->
		      #{id => {pitcp_acceptor, Ref, NumIndex},
			start =>
			    {pitcp_acceptor, start_link,
			     [Ref, NumIndex, ListenSocket, ProMod, ProModOpt,
			      OtherOpt, ListenerSup]},
