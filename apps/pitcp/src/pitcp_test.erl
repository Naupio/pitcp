...
run_listener() ->
    Ref = pitcp_test_ref,
    LisOpt = [binary, {port, 18080}, {packet, 0},
	      {active, false}, {ip, {127, 0, 0, 1}}],
    ProMod = pitcp_test,
    ProModOpt = [],
    OtherOpt = [],
    pitcp:start_listener(Ref, LisOpt, ProMod, ProModOpt,
			 OtherOpt).

spawn_conn(TestNum)
    when is_number(TestNum) andalso TestNum > 0 ->
    lists:foreach(fun (MsgNum) ->
			  spawn(fun () ->
					{ok, Socket} = pitcp_util:connect({127,
									   0, 0,
									   1},
									  18080,
									  [{active,
...
start_link(Ref, ClientSocket, ProModOpt, OtherOpt) ->
    gen_server:start_link(?MODULE,
			  [Ref, ClientSocket, ProModOpt, OtherOpt], []).

init([Ref, ClientSocket, ProModOpt, OtherOpt]) ->
    self() ! init,
    pitcp_util:setopts(ClientSocket, [{active, once}]),
    State = #{client_socket => ClientSocket, ref => Ref,
	      pro_mod_opt => ProModOpt, other_opt => OtherOpt},
    {ok, State}.

handle_call(get_client_socket, _From,
	    #{client_socket := ClientSocket} = State) ->
...
handle_cast({send, Data},
	    #{client_socket := ClientSocket} = State) ->
    pitcp_util:send(ClientSocket, Data), {noreply, State};
handle_cast(_Msg, _State) -> {noreply, _State}.

handle_info(init, _State) -> {noreply, _State};
handle_info({tcp, ClientSocket, Data},
	    #{client_socket := ClientSocket} = State) ->
    pitcp_util:setopts(ClientSocket, [{active, once}]),
